...
async function exportInventory(response: ApiResponse): Promise<void> {
  const [foods, lots, recipes, history, nutritionTargets, foodPreferences, externalFoods, plannedMeals, groceryItems, preparedBatches, mealTemplates, recipeFeedback] = await Promise.all([
