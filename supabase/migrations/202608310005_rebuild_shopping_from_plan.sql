create function public.rebuild_shopping_from_plan(
...
begin
  ...
  with planned_ingredients as (
    ...
    where shortage.shortage_base > 0.0000001
      and not exists (
        select 1 from public.shopping_items existing
        where existing.food = shortage.food
          and existing.source = 'generated'
          and existing.checked_at is not null
          and existing.lot is null
      )
  )
